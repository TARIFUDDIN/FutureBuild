...
const JobSessionView = ({ sessionId, initialJobs, session }) => {
  ...
  // Refresh jobs with polling for background scraping
  ...
  // Auto-refresh if no jobs found (background scraping might still be running)
  useEffect(() => {
    if (jobs.length === 0) {
      const pollForJobs = setInterval(async () => {
        try {
          const result = await getSessionJobs(sessionId);
          if (result.success && result.jobs && result.jobs.length > 0) {
            setJobs(result.jobs);
            clearInterval(pollForJobs);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* No Jobs State - Updated for background scraping */}
      {jobs.length === 0 && (
        <div className="text-center py-12">
          <div className="relative">
            <AlertCircle className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
            <div className="absolute -top-1 -right-1">
              <div className="animate-pulse bg-blue-500 h-3 w-3 rounded-full"></div>
            </div>
          </div>
          <h3 className="text-lg font-medium mb-2">Scraping jobs in progress...</h3>
          <p className="text-muted-foreground mb-4">
            We're finding real jobs from multiple sources. This typically takes 1-2 minutes.
            <br />
            <span className="text-sm">The page will automatically update when jobs are found.</span>
          </p>
          <div className="space-y-3">
            <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
              <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
              <span>Background scraping is active</span>
            </div>
            <Button onClick={handleRefreshJobs} disabled={refreshLoading} variant="outline">
              {refreshLoading ? (
                <Loader2 className="h-4 w-4 mr-2 animate-spin" />
              ) : (
                <RefreshCw className="h-4 w-4 mr-2" />
